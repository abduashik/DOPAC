...
{
	$sql="select * from patients where Email='{$_POST["email"]}' and Password='{$_POST["Pass"]}'";
	$res=$db->query($sql);
	if($res->num_rows>0)
	{
		$ro=$res->fetch_assoc();
		$_SESSION["ID"]=$ro["PatientID"];
		$_SESSION["USERNAME"]=$ro["Name"];
		$_SESSION["EMAIL"]=$ro["Email"];
		$_SESSION["CNUM"]=$ro["ContactNumber"];
		$_SESSION["AGE"]=$ro["Age"];
		$_SESSION["ADDRESS"]=$ro["Address"];
		$_SESSION["GENDER"]=$ro["Gender"];
		echo "<script>window.open('PatientHomeLogged.php','_self')</script>";
	}
	else
	{
...
<p id="registernow" style="margin-left:10px;font-size:20px">Not yet registered? <a href="PatientSignUp.php" style="color:blue;text-decoration:none;">Register Now!</a><p>
</div>
</body>
</html>